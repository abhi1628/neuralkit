...
// Import the renderContent function from Blog.js (or copy it)
// For now, we'll inline a simplified version that handles the types we use
...
function renderContent(block, i, theme) {
  ...
  const seriesColor = "#10b981"; // ML series color
...
export default function TutorialSeries({ theme }) {
  ...
  // In future, fetch series by slug. For now, we only have one series
  ...
  // Import the landing page content
  const landingPost = require(`../posts/${series.slug}-landing`).default;
  ...
  return (
    <div style={{ minHeight: "100vh", background: isDark ? "#08070f" : "#faf8ff", width: "100%" }}>
      ...
      {/* Parts Overview Sidebar + Content */}
      <div style={{ maxWidth: "800px", margin: "0 auto", padding: "48px 24px 80px", textAlign: "left" }}>
        ...
        {landingPost.content.map((block, i) => renderContent(block, i, theme))}

        {/* Start CTA */}
        <div style={{ marginTop: "48px", paddingTop: "32px", borderTop: `1px solid ${isDark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.08)"}` }}>
          <div style={{ textAlign: "center" }}>
            <a href={`/tutorials/${series.slug}/${series.parts[0].slug}`} style={{ display: "inline-block", background: "linear-gradient(135deg,#10b981,#059669)", color: "#fff", fontWeight: 700, fontSize: "1rem", padding: "14px 36px", borderRadius: "12px", textDecoration: "none", fontFamily: "'Space Mono',monospace" }}>

...
const RESUME_MODEL = "llama-3.3-70b-versatile";
  ...
  async function loadTrivia() {
    ...
    try {
      const res = await fetch(GROQ_API_URL, {
        ...
        body: JSON.stringify({
          model: GENERAL_MODEL, max_tokens: 300, temperature: 1.2,
          messages: [
  ...
  async function runTool() {
    ...
    try {
      const res = await fetch(GROQ_API_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ model: GENERAL_MODEL, max_tokens: 1000, messages: [{ role: "system", content: tool.systemPrompt }, { role: "user", content: sanitizedInput }] }),
      });
      const data = await res.json();
...
async function generateResume() {
    ...
    try {
      // ── STEP 1: Generate Improvement Plan (lightweight, ~3K tokens) ──
      const planRes = await fetch(GROQ_API_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...
          messages: [{
            role: "system",
            content: `You are an expert resume strategist. Analyze the provided resume weaknesses and create a concise, structured improvement plan.

Respond ONLY with valid JSON. No markdown, no explanation.

Format:
{
  "action_verbs": ["Led", "Architected", "Streamlined", "Optimized"],
  "metrics_to_add": ["quantify team size", "add % improvement", "add $ impact"],
  "missing_keywords": ["specific technical skills for target role"],
  "bullet_rewrites": [
    { "original": "weak bullet text", "improved": "STAR-method rewrite with metric" }
  ],
  "structure_fixes": ["move skills higher", "add achievements section"],
  "tone_adjustments": ["stronger leadership language", "more technical depth"]
}

Rules:
- Only suggest improvements based on the analysis feedback provided
- Do NOT invent fake experience or companies
- Focus on: quantification, action verbs, technical depth, leadership signals
- Keep bullet_rewrites to top 5 most impactful changes`
          }, {
            role: "user",
            content: `TARGET ROLE: ${jobTarget}
...
Generate the improvement plan as JSON now.`
          }]
        })
      });
      const planData = await planRes.json();
      const planRaw = planData?.choices?.[0]?.message?.content || "";
      const planCleaned = planRaw.replace(/```json|```/g, "").trim();
      let improvementPlan;
      try {
        improvementPlan = JSON.parse(planCleaned);
      } catch {
        improvementPlan = {
          action_verbs: ["Led", "Developed", "Implemented"],
          metrics_to_add: ["Add quantifiable results"],
          missing_keywords: [],
          bullet_rewrites: [],
          structure_fixes: [],
          tone_adjustments: ["Use stronger action verbs"]
        };
      }

      // ── STEP 2: Generate Enhanced Resume using plan + original text (~6K tokens) ──
      const resumeRes = await fetch(GROQ_API_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...
          messages: [{
            role: "system",
            content: `You are an elite resume writer. Apply the provided improvement plan to the original resume.
...
OUTPUT FORMAT:
{
  ...
  "achievements": ["Notable award"],
  "languages": ["Language (Proficiency)"],
  "ats_score": { "overall": 8.5, "keywords": 9, "format": 8, "impact": 8.5, "readability": 9 }
}

RULES:
1. Apply EVERY suggestion from the improvement plan
2. Use STAR method: Situation → Task → Action → Result with metrics
3. Start bullets with strong action verbs from the plan
4. Add quantifiable metrics where implied by original text
5. Include missing keywords naturally in bullets and skills
6. NEVER invent fake companies, degrees, or certifications
7. Keep all information factually grounded in the original resume`
          }, {
            role: "user",
...
Generate the enhanced resume as JSON now.`
          }]
        })
      });
      const resumeData2 = await resumeRes.json();
      const raw2 = resumeData2?.choices?.[0]?.message?.content || "";
  ...
  async function generate() {
    ...
    try {
      const res = await fetch(GROQ_API_URL, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          model: GENERAL_MODEL, max_tokens: 1200, temperature: 0.7,
          messages: [{
            role: "system",
  ...
  async function generate() {
    ...
    try {
      const res = await fetch(GROQ_API_URL, {
        ...
        body: JSON.stringify({
          model: GENERAL_MODEL, max_tokens: 2000,
          messages: [{
            role: "system",
  ...
  async function generateResume() {
    ...
    try {
      ...
      const planRes = await fetch(GROQ_API_URL, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          model: "llama-3.1-8b-instant", max_tokens: 800, temperature: 0.3,
          messages: [{
            role: "system",
            content: `You are an expert resume strategist. Analyze the provided resume weaknesses and create a concise, structured improvement plan.
...
Generate the improvement plan as JSON now.`
      ...
      const resumeRes = await fetch(GROQ_API_URL, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          model: "llama-3.1-8b-instant", max_tokens: 2000, temperature: 0.3,
          messages: [{
            role: "system",
            content: `You are an elite resume writer. Apply the provided improvement plan to the original resume.
  ...
  async function generate() {
    ...
    try {
      const res = await fetch(GROQ_API_URL, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          model: "llama-3.1-8b-instant", max_tokens: 1200, temperature: 0.7,
          messages: [{
            role: "system",
            content: `You are an expert career coach and cover letter writer. Write a compelling, ATS-friendly cover letter.

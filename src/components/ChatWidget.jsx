...
// Zer0 — Softened UX, no intimidating counters
...
export default function ChatWidget() {
  ...
  const { messages, isLoading, isLimited, sendMessage, resetChat, messageCount, activeContext } = useChat();
  ...
  const handleQuickReply = (text) => { if (isLoading || isLimited) return; sendMessage(text); };
      ...
      {!isOpen && (
        <button
          ...
          style={{
            ...
            border: 'none', color: '#fff', fontSize: '1.4rem', fontWeight: 700,
            cursor: 'pointer',
      ...
      {isOpen && (
        <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <div>
                ...
                <div style={{ fontSize: '0.7rem', color: textMuted, display: 'flex', alignItems: 'center', gap: '4px' }}>
                  <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: isLimited ? '#ef4444' : isLoading ? '#fbbf24' : '#22c55e', display: 'inline-block' }} />
          ...
          <div ref={scrollRef} style={{
            ...
            {messages.length === 0 && (
              <div style={{ textAlign: 'center', padding: '32px 8px', color: textMuted }}>
                <div style={{
                  width: '56px', height: '56px', borderRadius: '50%',
                  background: `linear-gradient(135deg, ${ac}, #818cf8)`,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                ...
                <div style={{ fontWeight: 700, fontSize: '1.05rem', color: text, marginBottom: '6px' }}>
                  Hi, I'm Zer0
                </div>
                <div style={{ fontSize: '0.85rem', lineHeight: 1.6, marginBottom: '4px' }}>
                  Zer0 is here to help
                </div>
                {/* Subtle hint, only when low */}
                {remaining <= 3 && remaining > 0 && (
                  <div style={{ fontSize: '0.7rem', opacity: 0.5, marginTop: '8px' }}>
                    A few messages left this visit
                  </div>
                )}

                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '20px' }}>
                  {currentReplies.map((reply, i) => (
                    <button key={i} onClick={() => handleQuickReply(reply.text)} disabled={isLoading || isLimited} style={{
                      ...
                      <span>{reply.icon}</span><span>{reply.text}</span>
            ...
            {messages.map((msg, i) => (
              <div key={i} style={{
                display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start', gap: '8px',
              }}>
                {msg.role === 'assistant' && (
                  <div style={{
                ...
                <div style={{
                  maxWidth: 'calc(100% - 40px)', padding: '10px 14px',
            ...
            {isLimited && (
              <div style={{
                ...
                <div style={{ fontWeight: 600, color: text, fontSize: '0.9rem', marginBottom: '4px' }}>
                  Great chat!
                </div>
                <div style={{ color: textMuted, fontSize: '0.85rem', marginBottom: '12px' }}>
                  Start fresh to keep exploring
                </div>
          ...
          <form onSubmit={handleSubmit} style={{
            ...
            <textarea
              ...
              placeholder={isLimited ? 'Start a new chat to continue' : 'Ask Zer0...'}
            ...
            />
            <button
              type="submit" disabled={!input.trim() || isLoading || isLimited}
              style={{

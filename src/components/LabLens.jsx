...
async function translateFields(englishResult, targetLang, callAI, primaryModel, fallbackModel, onProgress) {
  ...
  const fields = {
    ...
    // findings: array of [plain_meaning, significance, care_note] per entry
    findings: (englishResult.findings||[]).map(f =>
      `MEANING: ${f.plain_meaning}\nSIGNIFICANCE: ${f.significance}\nCARE: ${f.care_note||''}`
    ).join('\n---\n'),
          ...
          <div>
            ...
            {activeTab==='summary' && (
              <div style={{ display:'flex', flexDirection:'column', gap:'14px' }}>
                <div style={{ fontSize:'0.68rem', color:muted, lineHeight:1.6, padding:'0 2px' }}>The big picture — what your report means overall and what to do next.</div>
            ...
            {activeTab==='findings' && (
              <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
                <div style={{ fontSize:'0.68rem', color:muted, lineHeight:1.6, padding:'0 2px' }}>Every test explained — what it measures and what your specific number means.</div>
            ...
            {activeTab==='flagged' && (
              <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
                <div style={{ fontSize:'0.68rem', color:muted, lineHeight:1.6, padding:'0 2px' }}>Abnormal values only — what each one means for you and what you can do about it.</div>
                {flagged.length===0
                  ...
                  : flagged.map((f,i)=>{ const col=severityColor(f.fuzzy_label); return (
                    <div key={i} style={{ background:`${col}08`, border:`1px solid ${col}33`, borderRadius:'12px', padding:'18px' }}>
                      ...
                      <div style={{ fontSize:'0.84rem', color:text, lineHeight:1.7, fontFamily:scriptFont, marginBottom: f.care_note ? '10px' : 0 }}>{f.significance}</div>
                      {f.care_note && (
                        <div style={{ display:'flex', gap:'8px', alignItems:'flex-start', background:isDark?'rgba(16,185,129,0.06)':'rgba(16,185,129,0.05)', border:`1px solid rgba(16,185,129,0.2)`, borderRadius:'8px', padding:'10px 12px' }}>
                          <span style={{ color:green, flexShrink:0, fontSize:'0.85rem' }}>→</span>
                          <span style={{ fontSize:'0.82rem', color:isDark?'rgba(255,255,255,0.75)':'rgba(0,0,0,0.65)', lineHeight:1.65, fontFamily:scriptFont }}>{f.care_note}</span>
                        </div>
                      )}
                      {f.fuzzy_score>0 && <SeverityBar score={f.fuzzy_score} label={f.fuzzy_label} />}
                    </div>
            ...
            {activeTab==='questions' && (
              <div style={{ display:'flex', flexDirection:'column', gap:'14px' }}>
                <div style={{ fontSize:'0.68rem', color:muted, lineHeight:1.6, padding:'0 2px' }}>Specific questions for your doctor based on your actual results — not generic health questions.</div>
                <div style={{ background:card, border:`1px solid ${border}`, borderRadius:'14px', padding:'20px' }}>

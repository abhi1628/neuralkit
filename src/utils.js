// src/utils.js
import { DANGEROUS_INPUT_PATTERNS, DANGEROUS_OUTPUT_PATTERNS, GA_ID, VISITOR_API_URL } from './constants.js';
...
export async function downloadAsPDF(text, filename = 'zeroapi-output') {
  ...
  const doc = new jsPDF();
  const cleaned = text
    .replace(/[🎯🔍💡⚠️📌✅❌🚀📈◆]/g, m => ({ '🎯': '[CORE]', '🔍': '[FINDINGS]', '💡': '[INFO]', '⚠️': '[WARNING]', '📌': '[NOTE]', '✅': '[+]', '❌': '[-]', '🚀': '[KEY]', '📈': '[GROWTH]', '◆': '*' }[m] || m))

// api/subscribe.js — Production-ready with minimal logging
...
export default async function handler(req, res) {
  ...
  const base = `${SUPABASE_URL.replace(/\/$/, "")}/rest/v1/email_subscribers`;
  ...
  try {
    ...
    if (!insertRes.ok) {
      ...
      throw new Error(`Insert failed: ${insertRes.status} ${err?.message || err}`);
    }
